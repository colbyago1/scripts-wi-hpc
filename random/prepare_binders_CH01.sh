...
length_B=133
# msa path
msa_path="/home/cagostino/work/workspace/AbsBinders/msangster/AF/setup/CH01_CF/CH01_CF.a3m"
...
for ((i = 1; i <= 40; i++)); do
    ...
    # get sequence from binder
    seq=$(sed -n "${seq_num}p" $fa)
    name_raw=$(sed -n "${name_num}p" $fa)
    name="${name_raw#?}"
