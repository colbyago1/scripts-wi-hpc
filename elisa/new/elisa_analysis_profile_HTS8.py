...
fname='/Volumes/kulp/Colby/2024-02-08_plate6.xlsx' # CJA: output from plate reader, fetches data
lname='/Users/cagostino/Desktop/Desktop/ELISA/2024-02-08/Silent Trimer Antigen Characterization Plans.xlsx' # CJA: elisa template file, fetches antigen labels (requires that all 12 columns have a header)
...
for i in range(0,len(first_read.sheet_names)):  
    ...
    dfspk1_avg[dfspk1.columns[3]] = dfspk1.iloc[:,[3]].mean(axis=1)
    dfspk1_stddev[dfspk1.columns[3]] = dfspk1.iloc[:,[3]].std(axis=1)
    dfspk1_avg[dfspk1.columns[4]] = dfspk1.iloc[:,[4]].mean(axis=1)
    dfspk1_stddev[dfspk1.columns[4]] = dfspk1.iloc[:,[4]].std(axis=1)
    dfspk1_avg[dfspk1.columns[5]] = dfspk1.iloc[:,[5,6]].mean(axis=1)
    dfspk1_stddev[dfspk1.columns[5]] = dfspk1.iloc[:,[5,6]].std(axis=1)
    dfspk1_avg[dfspk1.columns[6]] = dfspk1.iloc[:,[7,8]].mean(axis=1)
    dfspk1_stddev[dfspk1.columns[6]] = dfspk1.iloc[:,[7,8]].std(axis=1)
    dfspk1_avg[dfspk1.columns[7]] = dfspk1.iloc[:,[9,10]].mean(axis=1)
    dfspk1_stddev[dfspk1.columns[7]] = dfspk1.iloc[:,[9,10]].std(axis=1)
    dfspk1_avg[dfspk1.columns[8]] = dfspk1.iloc[:,[11,12]].mean(axis=1)
    dfspk1_stddev[dfspk1.columns[8]] = dfspk1.iloc[:,[11,12]].std(axis=1)
    ...
    plot=sf.plotCurves_overlay(allprofiles,allfits,outdf,dfspk1_stddev,dfspk1_avg['conc'].values,name,'WellName',ymax=0.5,cols=4,logplot=True) # CJA: adjust ymax based on absorbance range
