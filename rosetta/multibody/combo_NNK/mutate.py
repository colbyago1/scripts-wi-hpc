...
def mutate(pose, posi, amino, partners, ID, wt_nrg, index, bool_relax):
    ...
    content=(pdbname+','+str(unbound)+','+str(binding/wt_nrg)+'\n')

    f = open(f'output_{index}.csv','a+')
    f.write(content)
    f.close()
...
parser.add_argument('--wt_nrg', dest = 'wt_nrg',
    default = 1.00,    # default 'G'
    help = 'the moving partners' )
...
mutate(pdb_filename, mutate_position, amino_acid, partners, identifier, wt_nrg, index, relax)
