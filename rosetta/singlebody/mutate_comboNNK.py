...
def mutate(pose, posi, amino, ID, bool_relax):
    ...
    content=(str(ID)+','+str(wt_nrg)+','+str(mut_nrg)+','+str(mut_nrg/wt_nrg)+'\n')
...
mutate_position = [int(i) for i in mutations]
...
if len(mutations) == 3:
    for a in aminos:
        amino_acid = [a, a, a]
        identifier = "_".join(amino_acid)
        print(amino_acid, mutate_position)
        mutate(pdb_filename, mutate_position, amino_acid, identifier, relax)
elif len(mutations) == 6:
    for a in aminos:
            for b in aminos:
                amino_acid = [a, b, a, b, a, b]
                identifier = "_".join(amino_acid)
                print(amino_acid, mutate_position)
                mutate(pdb_filename, mutate_position, amino_acid, identifier, relax)
