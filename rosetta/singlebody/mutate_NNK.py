...
def mutate(pose, posi, amino, ID, bool_relax):
    ...
    content=(str(amino)+','+str(wt_nrg)+','+str(mut_nrg)+','+str(mut_nrg/wt_nrg)+'\n')
...
for amino_acid in aminos:
    print(amino_acid)
    mutate_position = int(mutations[0])
    identifier = str(mutate_position) + amino_acid
