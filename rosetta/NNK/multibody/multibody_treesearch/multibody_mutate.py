...
def mutate(pdb_filename, posi, amino, partners, wt_nrg):
    ...
    content=(name+'.pdb'+','+str(unbound)+','+str(binding/wt_nrg)+'\n')
    fname = pdb_filename[:-4] + '.csv'
...
parser.add_option('--wt_nrg', dest = 'wt_nrg',
...
wt_nrg = float(options.wt_nrg)

mutate(pdb_filename, mutate_position, amino_acid, partners, wt_nrg)
