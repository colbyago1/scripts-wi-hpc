...
while IFS=',' read -r pdb_filename rest_of_line; do
    ...
    then
        ...
        echo "$/home/dwkulp/software/mslib.git/mslib/bin/alignMolecules --pdb1 $reference --pdb2 structures/$pdb_filename --sele1 name CA$selection --noOutputPdb"

        # Extract the last word from the output using awk
        rmsd=$(echo "$output" | awk '/RMSD/{print $2}')

        # if [ "$(echo "$rmsd > 12" | bc)" -eq 1 ]; then
        #     /home/dwkulp/software/mslib.git/mslib/bin/alignMolecules --pdb1 $reference --pdb2 structures/$pdb_filename --sele1 name CA$selection
        # fi
